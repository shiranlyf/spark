...
import com.ishiran.sparkproject.domain.AreaTop3Product;
...
//		sqlContext.setConf("spark.sql.shuffle.partitions", "1000"); //spark shuffle的并行度  默认是200
//		sqlContext.setConf("spark.sql.autoBroadcastJoinThreshold", "20971520"); //默认是10M  这是是(10485760)20M
...
//				+ "JOIN tmp_product_info pi ON tapcc.product_id=pi.product_id ";
	...
	private static JavaRDD<Row> getAreaTop3ProductRDD(SQLContext sqlContext) {
		...
		// 技术点：开窗函数(row_number() OVER (PARTITION BY area ORDER BY click_count DESC))
