...
import com.ishiran.sparkproject.domain.Top10Session;
import com.ishiran.sparkproject.test.MockData;
import com.ishiran.sparkproject.util.DateUtils;
import com.ishiran.sparkproject.util.NumberUtils;
import com.ishiran.sparkproject.util.ParamUtils;
import com.ishiran.sparkproject.util.SparkUtils;
import com.ishiran.sparkproject.util.StringUtils;
import com.ishiran.sparkproject.util.ValidUtils;
 ...
 *	DELETE FROM `top10_session`;
 *	
 *	SELECT * FROM `session_detail` WHERE session_id = '39308af8719a41cdac597b3c72f3b947' ORDER BY action_time;
 *
 *
 * 第一个模块的简单总结：
 *
...
public class UserVisitSessionAnalyzeSpark {
	...
	public static void main(String[] args) {
		...
		//{"startAge":["0"], "endAge":[50],"startDate":["2018-06-03"],"endDate":["2018-06-03"]}
		//args = new String[]{"1"};  
...
//				.setMaster("local")
//				.set("spark.default.parallelism", "100")  //cpu core总数的2~3倍
				.set("spark.storage.memoryFraction", "0.5")  //cache 内存占有率    默认是0.6
				.set("spark.shuffle.consolidateFiles", "true") //每个节点的文件进行合并   有几个cpu core 就有几个文件
				.set("spark.shuffle.file.buffer", "64")  //task map写入磁盘的大小  默认是64kb
				.set("spark.shuffle.memoryFraction", "0.3") //task reduce shuffle 拉去过来内存占比
				.set("spark.reducer.maxSizeInFlight", "24")  //reduce缓冲区的大小  默认是48m
				.set("spark.shuffle.io.maxRetries", "60") //shuffle 拉去数据文件的时候重复拉去的次数
				.set("spark.shuffle.io.retryWait", "60")   //shuffle 拉去数据文件的时候重复拉去的次数  时间间隔
		...
		/**
		 * 比如， 获取top10热门品类功能中， 二次排序，自定义了Key
		 * 那个key是需要在进行shuffle的时候，进行网络传输的， 因此也是要求实现序列化的
		 * 启动Kryo机制以后，就会用Kryo去序列化和反序列化CategorySortKey
		 ...
		 * 
		 * sessionid为key的PaiRDD,是一个确定了，在后面多次使用的
		 * 1、与通过筛选了sessionid进行join,获取通过筛选的session的明细数据
		...
		/**
		 *  持久化：sessionid2ActionRDD在下面被使用了两次    所以这里使用了两次  使用存内存的方式
		 *  
		 *  持久化的级别 ：
		 *  ① StorageLevel.MEMORY_ONLY(): 表示存内存   无序列化   就可以用cache方法进行替代
		 *  ② StorageLevel.MEMORY_ONLY_SER() 内容序列化
		 *  ③ StorageLevel.MEMORY_AND_DISK() 内存磁盘没有序列化
		 *  ④ StorageLevel.MEMORY_AND_DISK_SER() 内存序列化
		 *  ⑤ StorageLevel.DISK_ONLY() 纯粹磁盘的方式
		 *  
		 *  如果内存充足  可使用双副本高可靠机制
		 *  选择_2的级别
		 *  
		...
		//打印RDD
		//printJavaPairRDDInfo(sessionid2AggrInfoRDD);
		...
		/**
		 * 持久化：filteredSessionid2AggrInfoRDD被使用了两次   所以进行持久化
		 */
		filteredSessionid2AggrInfoRDD = filteredSessionid2AggrInfoRDD.persist(StorageLevel.MEMORY_ONLY());
		
		//公共的RDD:通过筛选的session的访问明细数据
		/**
		 * 重构：sessionid2DetailRDD, 就是代表了通过筛选session对应的访问明细数据
		 */
		JavaPairRDD<String, Row> sessionid2detailRDD = 
				getSessionid2detailRDD(filteredSessionid2AggrInfoRDD, sessionid2ActionRDD);
		
		/**
		 * sessionid2detailRDD被使用了三次   所以进行持久化
		 */
		 ...
		 */
		//计算出各个范围的session占比  并写入到Mysql
		calculateAndPersistAggrStat(sessionAggrStatAccumulator.value(), task.getTaskid());
		
		
		/**
		 * session聚合统计（统计访问时长和访问步长，各个区间session数量占总session数据的比例）
		 */
		//获取top10热门品类
	 ...
	 * @param sessionid2aggerInfoRDD
	 * @param sessionid2ActionRDD
	 * @return
	 */
	private static JavaPairRDD<String, Row> getSessionid2detailRDD(
			JavaPairRDD<String, String> sessionid2aggerInfoRDD,
			JavaPairRDD<String, Row> sessionid2ActionRDD){
		JavaPairRDD<String, Row> sessionid2DetailRDD = sessionid2aggerInfoRDD
	...
	private static void randomExtractSession(
			...
			JavaPairRDD<String, String> sessionidAggrInfoRDD, 
			JavaPairRDD<String, Row> sessionid2ActionRDD) {
		/**
		 * 第一步，计算出每个小时session的数量  
		 */
		
		//获取<yyyy-MM-dd_HH,sesssionid>格式的RDD
		JavaPairRDD<String, String> time2sessionidRDD = sessionidAggrInfoRDD.mapToPair(
		...
		//开始按时间闭了进行计算 抽取算法  进行计算
		//总共抽取100个session  按照天数进行平分
		...
		for(Map.Entry<String, Map<String, Long>> dateHourCountEntry :
			dateHourCountMap.entrySet()){
				...
				for(Map.Entry<String, Long> hourCountEntry : hourCountMap.entrySet()){
					...
					// 计算每个小时的session的数量，占据当天总session数量的比例， 截止乘以每天要抽取的数量
		...
		/**
		 * fastutil的使用：List<Integer>  改写成IntList
		 */
		 ...
		 * session随机抽取功能
		 * 用到了比较大的变量Map   随机抽取索引   
		 * 之前是在算子里面使用map  每个task会拷贝一份map
		 * 
		 * 将Map做成广播变量
		 * 
		 * 使用方法：
		 * JavaSparkContext 的broadcast传入需要传播的变量即可
		 */
		...
		//使用flatMap这种算子遍历<dateHour, (session aggrInfo)>格式的数据
		//然后会遍历每天每小时的session
		//如果发现某个sesson恰巧在我们制定的这天这个小时的随机抽取索引上
		//那么抽取改session,直接写入MYSQL的random_extract_session表
		//将抽取出来的session id返回回来，行程一个新的JavaRDD<String>
		//然后最后一步将用抽取出来的sessionid， 去join它们的访问行为明细数据 然后写入到session
		JavaPairRDD<String, String> extractSessionidsRDD = time2sessionsRDD.flatMapToPair(new PairFlatMapFunction<Tuple2<String,Iterable<String>>, String, String>() {
			...
			public Iterable<Tuple2<String, String>> call(Tuple2<String, Iterable<String>> tuple) throws Exception {
				List<Tuple2<String, String>> extractSessionds = new ArrayList<Tuple2<String,String>>();
				 ...
				 * 直接调用广播变量中value的值
				 * 
				 * 同一个Excutor（管理资源或者内存）下的task  task调用的时候Excutor上是否有Map变量   
				 * 没有的话  从Driver上进行获取
				 */
				Map<String, Map<String, IntList>> dateHourExtractMap = dateHourExtractMapBroadcast.getValue();
				List<Integer> extractIndesList = dateHourExtractMap.get(date).get(hour);
				...
				while (iterator.hasNext()) {
					...
					if(extractIndesList.contains(index)){
						//将数据写入到MYSQL
						SessionRandomExtract sessionRandomExtract = new SessionRandomExtract();
						sessionRandomExtract.setTaskid(taskid);
						sessionRandomExtract.setSesssionid(sessionid);
						...
						//将sessionid加入list
						extractSessionds.add(new Tuple2<String, String>(sessionid, sessionid));
					}
					
					index++;
				}
				return extractSessionds;
			}
		});
	 ...
	 * @param rdd 参数
	 */
	private static void printJavaPairRDDInfo(JavaPairRDD<String, String> rdd){
		System.out.println(rdd.count());
		for(Tuple2<String, String> tuple : rdd.take(10)){
...
//		return actionDF.javaRDD().repartition(1000);
	...
	private static JavaPairRDD<String, String> aggregateBySession(
		 ...
		 * 
		 * broadcast广播出去之后Excutor BlockManager中取   
		 *   Excutor BlockManager没有会获取进行  第二个task就不会进行获取了
		 */
...
//				});
		 ...
		 * 
		 * 适合用在一个key 或几个key少量key倾斜的场景
		 * 
		 * 采样倾斜的key的RDD在一个task上时    在进行join的时候就会分散到不同task上
		 */
...
//				});
		...
		JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD = sessionid2AggrInfoRDD.filter(
				...
				new Function<Tuple2<String,String>, Boolean>() {
					...
					public Boolean call(Tuple2<String, String> tuple) throws Exception {
						...
						//如果经过之前的多个过滤条件之后   程序能够走到这里
						//那么说明，改session是通过用户指定的帅选条件的  也是需要保留的session
						// 那么就要对session的访问时长和访问步长进行计算  根据session对应的
						//范围进行累加计数
					...
					private void calculateStepLength(long stepLength){
						if(stepLength >= 1 && stepLength <= 3){
							sessionAggrStatAccumulator.add(Constants.STEP_PEPIOD_1_3);
						}else if(stepLength >= 4 && stepLength <= 6){
							sessionAggrStatAccumulator.add(Constants.STEP_PEPIOD_4_6);
	...
	private static void calculateAndPersistAggrStat(String value, long taskid) {
		...
		long step_length_1_3 = Long.valueOf(StringUtils.getFieldFromConcatString(
				value, "\\|", Constants.STEP_PEPIOD_1_3));
		long step_length_4_6 = Long.valueOf(StringUtils.getFieldFromConcatString(
				value, "\\|", Constants.STEP_PEPIOD_4_6));
		...
		// 调用对应的DAO插入统计结果
		ISessionAggrStatDAO sessionAggrStatDAO = DaoFactory.getSessionAggrStarStatDAO();
	...
	private static List<Tuple2<CategorySortKey, String>> getTop10Category(long taskid,JavaPairRDD<String, Row> sessionid2DetailRDD) {
		/**
		 *  获取所有符合条件session访问过所有品类
		 */
		//获取session访问过的所有品类id
		// 访问过：指的是，点击过，下单过，支付过的品类
		//一个sessionid对应多个品类使用用flatMapToPair
...
//				});
	 ...
	 * @param sessionid2detailRDD
	 */
	private static void getTop10Session(JavaSparkContext sc, final long taskid, List<Tuple2<CategorySortKey, 
			String>> top10CategoryList, JavaPairRDD<String, Row> sessionid2detailRDD) {
		/**
		 * 第一步：将top10热门品类的id,生成一分RDD
		 */
	     ...
	     * 每一个session对应的访问明细数据就是一个集合
	     */
		JavaPairRDD<String, Iterable<Row>> sessionid2detail2RDD = 
				sessionid2detailRDD.groupByKey();
		
		JavaPairRDD<Long, String> categoryid2sessionCountRDD = sessionid2detail2RDD.flatMapToPair(
		...
		JavaPairRDD<Long, String> top10CateogrySessionCountRDD = 
		...
		/**
		 * 第三步： 分组去TopN算法实现， 获取每个品类的top10活跃用户
		 */
		JavaPairRDD<Long, Iterable<String>> top10CategorySessionCountsRDD = 
				top10CateogrySessionCountRDD.groupByKey();
		...
		/**
		 * 第四步：获取top10 session活跃的明细数据   写入到MYSQL
		 */
